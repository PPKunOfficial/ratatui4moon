...
v0.1：`core` / `backend` / `widgets` 三包绿；Rect/Style/Cell/Buffer/
Span/Line 与 Block（边框/线型/标题/内边距）的单元测试逐条复刻自
ratatui-v0.30.2 并 TDD 转绿（含 CJK 差分尾列回归规格与 emoji 截断
语义）；黄金快照管线已立。
设计全文见 [docs/design.md](docs/design.md)，仓库条约见 [AGENTS.md](AGENTS.md)。
