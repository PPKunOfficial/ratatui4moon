...
- **v0.1（当前）**：core（Buffer/diff/Style/text 基元/symbols line+block+
  bar+shade 符号表）+ backend（trait/TestBackend/ANSI）+ widgets（Block/
  Paragraph/List/Tabs/Gauge/LineGauge/Sparkline），上游 v0.30.2 对应模块的
  单测与集成黄金用例逐条复刻全绿 ✅
- **v0.2**：`unicode/` 宽度表全量搬运与折行升级；线性 `layout/`
  （`LayoutSolver` trait + LinearSolver）；Scrollbar/Table 续推；
